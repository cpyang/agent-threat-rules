...
AI assistants like ChatGPT, Claude, and Copilot can now browse the web, run code, read your files, and use external tools. This makes them incredibly useful -- but it also means **attackers can trick them into doing harmful things**: leaking your data, running malicious commands, or ignoring their safety instructions.

AI 助理（如 ChatGPT、Claude、Copilot）現在可以瀏覽網頁、執行程式碼、讀取檔案、使用外部工具。這讓它們非常強大 -- 但也意味著**攻擊者可以欺騙它們做有害的事**：洩漏資料、執行惡意指令、或繞過安全限制。

**ATR is a collection of "detection rules" that spot these tricks.** Think of it as a phrasebook of known attack patterns. Any security tool can load these rules and check whether an AI agent is being manipulated -- in real time, before damage is done.

**ATR 是一套偵測這些攻擊手法的「規則集」。** 把它想像成一本已知攻擊模式的對照手冊。任何安全工具都可以載入這些規則，即時檢查 AI Agent 是否正在被操控 -- 在造成傷害之前。

**For security professionals:** ATR is the Sigma/YARA equivalent for AI agent threats -- a YAML-based, open-source detection format with regex pattern matching, behavioral fingerprinting, and embedded test cases. Maps to OWASP LLM Top 10, OWASP Agentic Top 10, and MITRE ATLAS.

**給資安專業人士：** ATR 是 AI Agent 威脅的 Sigma/YARA -- 基於 YAML 的開源偵測格式，支援正則匹配、行為指紋、內建測試案例，對應 OWASP LLM Top 10、OWASP Agentic Top 10 和 MITRE ATLAS。
...
- **[OWASP](https://owasp.org/)** (security industry standards body) names the risks, but provides no executable detection rules
- **[MITRE ATLAS](https://atlas.mitre.org/)** (AI attack knowledge base) catalogs attack techniques, but offers no detection format
- **Real vulnerabilities are already being exploited**: CVE-2025-53773 (Copilot remote code execution), CVE-2025-32711 (EchoLeak data exfiltration), CVE-2025-68143 (MCP server exploit)
...
ATR (Agent Threat Rules) is a YAML-based detection format for AI agent threats. Inspired by what **[Sigma](https://github.com/SigmaHQ/sigma)** did for server log analysis and **[YARA](https://github.com/VirusTotal/yara)** for malware scanning, ATR aims to become the shared language for detecting:

- **Prompt injection** -- tricking an AI by hiding instructions in text ("ignore previous instructions and...")
- **Tool poisoning** -- making AI tools return malicious content that manipulates the agent
- **MCP exploitation** -- abusing the [Model Context Protocol](https://modelcontextprotocol.io/) (the standard way AI tools connect to external services)
- **Agent manipulation** -- one AI agent deceiving or hijacking another
- **Context exfiltration** -- stealing an AI's internal instructions, API keys, or conversation data

But we're just getting started.

ATR 是一種用於 AI Agent 威脅的 YAML 偵測格式。就像 **Sigma** 定義了伺服器日誌偵測規則、**YARA** 定義了惡意程式特徵，ATR 希望成為以下威脅的共通偵測語言：

- **Prompt injection** -- 在文字中藏指令來欺騙 AI（「忽略之前的指示然後...」）
- **Tool poisoning** -- 讓 AI 工具回傳惡意內容來操控 Agent
- **MCP 攻擊** -- 濫用 Model Context Protocol（AI 工具連接外部服務的標準協議）
- **Agent 操控** -- 一個 AI Agent 欺騙或劫持另一個
- **Context 外洩** -- 竊取 AI 的內部指令、API 金鑰或對話資料
...
Every ATR rule is a YAML file (a human-readable text format). Each rule answers four questions:

1. **What attack does this detect?** -- a title, description, and severity level
2. **How does it detect it?** -- text patterns to look for (like "ignore previous instructions")
3. **What should happen when detected?** -- block, alert, quarantine, or escalate
4. **How do we know it works?** -- built-in test cases that prove the rule catches attacks and does not trigger on normal text

每條 ATR 規則都是一個 YAML 檔案（人類可讀的文字格式）。每條規則回答四個問題：

1. **偵測什麼攻擊？** -- 標題、描述、嚴重程度
2. **怎麼偵測？** -- 要尋找的文字模式（如「忽略之前的指示」）
3. **偵測到之後怎麼辦？** -- 阻擋、警報、隔離、或升級處理
4. **怎麼知道它有效？** -- 內建測試案例，證明規則能抓到攻擊且不會誤判正常文字

Here's a real example from our current rule set:
...
We currently have rules across 9 categories, mapped to industry security standards. There are gaps -- and we need help filling them.

目前我們有 9 大類別的規則，對應到業界安全標準。還有很多空白需要填補。

> **Reading the table below:** "OWASP LLM" and "OWASP Agentic" are industry risk catalogs (like a menu of known dangers). "MITRE ATLAS" is an encyclopedia of AI attack techniques. "CVE" numbers are specific, confirmed vulnerabilities. You don't need to know these to use ATR -- they help security teams map rules to existing frameworks.
>
> **怎麼看下面的表格：** OWASP LLM 和 OWASP Agentic 是業界風險目錄（列出已知危險的清單）。MITRE ATLAS 是 AI 攻擊技術百科。CVE 編號是已確認的特定漏洞。使用 ATR 不需要了解這些 -- 它們幫助資安團隊對應到現有框架。
