...
\textbf{(5)} We present Agent Threat Rules (ATR) \cite{atr}, the first open-source implementation of this standard, and validate it empirically: \textbf{113 YAML-based detection rules} across 9 threat categories, independently benchmarked at 99.6\% precision and 62.7\% recall against the external PINT dataset \cite{pint} (850 samples), with 100\% recall and 97\% precision on the SKILL.md structured content benchmark (498 samples, 0.20\% FP rate). An ecosystem-wide scan of \textbf{96,096 agent skills} across six registries (OpenClaw, ClawHub, Skills.sh, Hermes, MCP Registry) discovered \textbf{751 active malware skills} from three coordinated threat actors \cite{atrscanmega}. A threat crystallization flywheel has been validated end-to-end: community signal aggregation $\rightarrow$ LLM-assisted rule generation $\rightarrow$ automated quality gate $\rightarrow$ human review $\rightarrow$ publication. ATR rules are shipped as upstream detection by Cisco AI Defense \cite{ciscopr79}.
...
\textbf{PINT (external)} & \textbf{850} & \textbf{99.6\%} & \textbf{62.7\%} & \textbf{77.1\%} & \textbf{0.25\%} \\
...
ATR achieves 100\% recall on structured SKILL.md content and 62.7\% on adversarial natural language (PINT). This gap is informative, not embarrassing:
...
\textbf{Methodology:} Scanned 96,096 skills across six registries: OpenClaw (56,480 skills), ClawHub (36,498), Skills.sh (3,115), Hermes Agent (123), and MCP Registry (4,880). Each skill was evaluated against all 113 ATR rules. Findings were reported to Threat Cloud for community review and blacklisting.
...
\textbf{Malware campaign discovery.} The scan uncovered the largest documented malware campaign targeting AI agent ecosystems: \textbf{751 active malware skills} distributed by three coordinated threat actors:
...
All 751 skills were reported to NousResearch (hermes-agent\#9809), blacklisted in Threat Cloud (554 entries), and are documented in detail in the companion research report \cite{atrscanmega}.

\textbf{Top detection:} Rule ATR-2026-00121 (credential access via tool description) triggered 686 times, accounting for 53\% of all flagged skills. Tool description poisoning --- embedding hidden instructions in a skill's metadata that the AI agent processes as trusted input --- remains the dominant attack vector.

\textbf{False positive analysis.} Of the 1,302 flagged skills, 751 are confirmed malware. The remaining $\sim$551 flagged skills (0.57\% of all skills) include over-privileged but non-malicious skills and legitimate security tools. The benchmark FP rate (0.20\% on labeled corpus) is consistent with the wild scan FP rate. ATR operates in report-only mode --- flagged skills are not blocked by default, preserving agent automation workflows.
...
We have implemented these requirements as ATR \cite{atr} --- 113 open-source detection rules, a 5-tier cascade engine, and a validated threat crystallization flywheel --- and tested the approach against three benchmarks: PINT \cite{pint} (99.6\% precision, 62.7\% recall), SKILL.md (100\% recall, 97\% precision, 0.20\% FP rate), and an ecosystem-wide scan of 96,096 agent skills \cite{atrscanmega} across six registries. The scan discovered 751 active malware skills from three coordinated threat actors --- the largest documented malware campaign targeting AI agent ecosystems.

Beyond the rule corpus, we publish RFC-001 \cite{rfc001} --- the first vendor-neutral quality standard for AI agent detection rules. RFC-001 defines maturity levels, confidence scoring, two-dimensional compliance (metadata $\times$ provenance), review tier levels L0--L5, community signal aggregation, and multi-runtime compatibility across 14 agent platforms. The standard is explicitly designed to be adopted without adopting ATR's rule format: any vendor implements one adapter interface and gains comparable quality scores. Microsoft's CTI-REALM benchmark \cite{ctirealm}, published concurrently in March 2026, independently validates that AI agents can generate detection rules from threat intelligence --- the same mechanism ATR's crystallization flywheel uses in production.

ATR is no longer just a proposal. Cisco AI Defense ships 34 ATR rules in production \cite{ciscopr79}. OWASP accepted ATR attack examples into the Agentic Top 10 working group. The EU AI Act enters full enforcement in August 2026 \cite{euaiact} --- enterprises deploying autonomous agents will need detection controls that map to regulatory frameworks. ATR's rules map to all 10 OWASP Agentic categories, 100 of 113 rules map to MITRE ATLAS techniques, and RFC-001's maturity model provides the audit trail regulators require.

The Snyk acquisition of Invariant Labs \cite{snykinvariant} --- creators of the leading MCP scanning tool --- validates the market but makes the case for an open standard more urgent: when the leading open-source scanner becomes proprietary, the community needs a standard that cannot be acquired away. ATR's permanent MIT license commitment, published in GOVERNANCE.md, ensures this.

The collapse of trust is happening now. Every CVE, every incident, every benchmark result in this paper is from the last 12 months. The 751 malware skills we discovered are still being cleaned up. The MCP SDK downloads 97 million times per month. The EU AI Act takes effect in four months. ATR is one answer --- and it is being adopted. The industry needs many more.
