...
\textbf{(5)} We present Agent Threat Rules (ATR) \cite{atr}, the first open-source implementation of this standard, and validate it empirically: 108 YAML-based detection rules across 9 threat categories, independently benchmarked at 99.6\% precision and 61.4\% recall against the external PINT dataset \cite{pint} (850 samples), with 96.9\% recall and 100\% precision on the SKILL.md benchmark \cite{atrskillbench} (498 real-world skill samples). A mega scan of the AI agent skill ecosystem \cite{atrmegscan} (53,577 skills across OpenClaw and Skills.sh) flagged \textbf{946 skills (1.77\%), with 875 rated CRITICAL.} ATR has been adopted as upstream detection rules by Cisco AI Defense \cite{ciscopr}.
\end{abstract}
...
Regex-based detection catches the first form and misses the rest. Our evaluation data confirms this: Tier 2 pattern matching achieves 61.4\% recall on PINT's external benchmark \cite{pint}. However, the SKILL.md benchmark \cite{atrskillbench} reveals a more nuanced picture: against structured skill descriptions (as opposed to free-form natural language), ATR achieves 96.9\% recall --- suggesting that the regex ceiling depends heavily on content structure. For structured, semi-formal content like tool descriptions and SKILL.md files, pattern matching is far more effective than for arbitrary natural language.
...
\textbf{Tier 2 (Pattern Detection):} 108 YAML rules with pre-compiled regex, matching known attack signatures across 6 agent data flow points: \texttt{llm\_io}, \texttt{tool\_call}, \texttt{mcp\_exchange}, \texttt{multi\_agent\_comm}, \texttt{context\_window}, and \texttt{memory\_access}. Median latency: 3.3ms.
...
\midrule
Prompt Injection & 33 & Agent's reasoning & llm\_io, tool\_call, context\_window \\
Tool Poisoning & 22 & Agent's tools (MCP) & tool\_call, mcp\_exchange \\
Skill Compromise & 22 & Agent's dependencies & tool\_call, mcp\_exchange \\
Context Exfiltration & 14 & Agent's knowledge & llm\_io, memory\_access \\
Privilege Escalation & 8 & Agent's permissions & tool\_call, context\_window \\
Agent Manipulation & 5 & Inter-agent trust & multi\_agent\_comm \\
Excessive Autonomy & 2 & Agent's boundaries & tool\_call, context\_window \\
Model Security & 1 & Model itself & llm\_io \\
...
\begin{itemize}
\item \textbf{OWASP Agentic Top 10 (2026)}: 10/10 full coverage \cite{owaspagentic}
\item \textbf{OWASP LLM Top 10 (2025)}: 7/10 full coverage \cite{owaspllm}
\item \textbf{OWASP Skills Top 10}: 7/10 coverage
\item \textbf{SAFE-MCP (OpenSSF)}: 78/85 controls = 91.8\% coverage \cite{safemcp}
\end{itemize}


\subsection{Benchmark Results}

We evaluated ATR against internal benchmarks and two external benchmarks:
...
\textbf{SKILL.md (external)} & \textbf{498 (32 mal + 466 benign)} & \textbf{100\%} & \textbf{96.9\%} & \textbf{98.4\%} & \textbf{0\%} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Self-test corpus:} 341 samples written alongside ATR rules. Difficulty-stratified. 824 embedded tests pass across all 108 rules. Latency: p50=3.72ms, p95=8.69ms, mean=4.34ms.

\textbf{PINT benchmark:} 850 samples from Lakera's external prompt injection dataset \cite{pint} --- questions ATR has never seen. The 0.25\% false positive rate (1 in 400) means ATR can run on production traffic without alert fatigue. Recall improved from 39.9\% (v0.3) to 61.4\% (v1.1) through 47 additional rules and threat crystallization.

\textbf{SKILL.md benchmark:} 498 real-world skill descriptions \cite{atrskillbench} --- 32 confirmed malicious and 466 confirmed benign samples sourced from OpenClaw, Skills.sh, and known attack repositories. Layer A (critical threats: credential exfiltration, hidden instructions, consent bypass): 100\% recall (24/24). Layer C (subtle threats: social engineering, gradual escalation): 87.5\% recall (7/8). Zero false positives across all 466 benign samples. This benchmark demonstrates that ATR's pattern-matching approach is highly effective against structured skill content, where attack patterns must use specific keywords and syntax to be functional.


\subsection{Why Transparent Recall Provides More Utility Than Opaque 98\%}

The gap between PINT recall (61.4\%) and SKILL.md recall (96.9\%) is itself informative:

\begin{lstlisting}
PINT (61.4% recall):
  - Free-form natural language attacks
  - Paraphrase, encoding, language switching
  - Regex structurally limited against NL

SKILL.md (96.9% recall):
  - Structured skill descriptions (YAML, JSON, Markdown)
  - Attacks must use functional keywords to work
  - Pattern matching highly effective
\end{lstlisting}

This tells us WHERE regex works and where it doesn't. Against structured content (tool descriptions, skill files, MCP metadata), pattern matching achieves near-perfect recall. Against free-form natural language, it needs Tier 4 semantic analysis.
...
\subsection{Mega Scan: AI Agent Skill Ecosystem}

On April 8, 2026, we used ATR v1.1.1 to conduct a comprehensive scan of the AI agent skill ecosystem \cite{atrmegscan}. This is, to our knowledge, the largest automated security audit of AI agent skills to date.

\textbf{Methodology:} Scanned 53,577 skills across two major registries: OpenClaw (50,485 skills) and Skills.sh (3,115 skills). Each skill's description, tool definitions, and metadata were evaluated against all 108 ATR rules.

\textbf{Results:}

\begin{table}[htbp]
\centering
\caption{Mega Scan: AI Agent Skill Ecosystem (April 2026)}
\label{tab:megascan}
\begin{tabular}{lrr}
\toprule
\textbf{Metric} & \textbf{Count} & \textbf{Percentage} \\
\midrule
Total skills scanned & 53,577 & 100\% \\
Skills flagged & 946 & \textbf{1.77\%} \\
CRITICAL & 875 & 1.63\% \\
HIGH & 52 & 0.10\% \\
MEDIUM & 19 & 0.04\% \\
Clean & 52,631 & 98.23\% \\
...
\textbf{ATR rule effectiveness:} The top-triggering rule (ATR-2026-00121) fired 674 times across the corpus, followed by ATR-2026-00099 and ATR-2026-00061. 926 threat reports were generated and pushed to the ATR Threat Cloud for community analysis.

\textbf{Comparison with v0.3 npm scan:} The earlier npm MCP scan (March 2026, v0.3.1) of 2,386 packages found 49\% with findings and 27\% HIGH or CRITICAL. The mega scan's lower flagging rate (1.77\%) reflects two factors: (1) the v1.1.1 ruleset is more precise, reducing false-positive triggering on benign patterns, and (2) the mega scan evaluated skill descriptions rather than full package source code. The critical finding remains: nearly 1,000 skills in the wild exhibit patterns consistent with malicious behavior.


\subsection{Ecosystem Adoption}

ATR is no longer a standalone project. It is being adopted as upstream infrastructure by production security systems:

\begin{itemize}
\item \textbf{Cisco AI Defense} merged 34 ATR rules as upstream detection content into their skill-scanner module (PR \#79) \cite{ciscopr}. This represents the first adoption of ATR by a major security vendor.
\item \textbf{3 ecosystem PRs merged, 7 under review} across OWASP, Cisco, awesome-lists, and other security projects, covering repositories with 90,000+ combined GitHub stars.
\item \textbf{npm distribution}: 23,000+ downloads per month across ATR-related packages.
\end{itemize}

This adoption pattern validates the core thesis: the industry needs an open detection standard, and ATR's format (YAML rules, framework mappings, mandatory evasion documentation) is compatible with existing security toolchains.
...
Next time the same attack pattern appears, Tier 2 catches it in $<$5ms. No LLM call needed. The expensive detection becomes a cheap detection. The system hardens itself over time through this process.

This mechanism has been validated at scale: the mega scan of 53,577 skills generated 926 threat reports, which were pushed to the ATR Threat Cloud. From these reports, 42 crystallization proposals were generated --- new pattern rules derived from semantic analysis of novel attack variants. Combined with community-driven threat intelligence (where crystallized rules from one deployment are shared anonymously with all deployments), this creates the flywheel effect described in Requirement 4: more agents scanning $\rightarrow$ more novel attacks caught $\rightarrow$ more rules crystallized $\rightarrow$ better defense $\rightarrow$ attracts more users $\rightarrow$ more agents scanning. The improvement from 39.9\% to 61.4\% PINT recall between v0.3 and v1.1 is direct evidence of this flywheel in operation.


\subsection{Known Limitations}

\textbf{Regex ceiling depends on content structure.} Against free-form natural language attacks (PINT benchmark), Tier 2 pattern matching reaches approximately 61.4\% recall. Paraphrase, language switching, and adversarial token manipulation are structurally difficult for regex. However, the SKILL.md benchmark (96.9\% recall) demonstrates that the regex ceiling is significantly higher for structured content --- tool descriptions, skill metadata, and configuration files --- where attacks must use specific functional keywords to be effective. ATR's response: use regex for what it's good at (structured content, known patterns, near-zero false positives, $<$5ms), and escalate free-form content to Tier 4.

\textbf{64 known evasion techniques documented.} ATR maintains an explicit catalog of 64 evasion techniques that can bypass Tier 2 detection, including Unicode homoglyphs, Base64 encoding, language switching, multi-turn decomposition, and context-dependent triggers. Each technique is documented with examples and mapped to the tier required for detection. This catalog is public and continuously updated.
...
\textbf{Rule supply chain.} ATR rules are YAML text. An attacker who can modify rule files can disable detection. Planned mitigation: Sigstore rule signing (roadmap).
...
The infrastructure exists to attack AI agents at scale. The infrastructure to defend them is only now being built.
...
We have implemented these requirements as ATR \cite{atr} --- 108 open-source detection rules across 9 threat categories, a 5-tier cascade engine, and a community-driven threat intelligence mechanism --- and validated the approach against two external benchmarks: PINT \cite{pint} (99.6\% precision, 61.4\% recall on 850 free-form injection samples) and SKILL.md \cite{atrskillbench} (100\% precision, 96.9\% recall on 498 real-world skill descriptions). A mega scan of 53,577 AI agent skills \cite{atrmegscan} flagged 946 (1.77\%) with patterns consistent with malicious behavior. The threat crystallization flywheel is operational: 926 threat reports generated, 42 proposals crystallized, recall improving measurably between releases.

The adoption of ATR by Cisco AI Defense as upstream detection rules \cite{ciscopr} validates that the format works for production security toolchains. The 10/10 OWASP Agentic Top 10 coverage and 91.8\% SAFE-MCP coverage \cite{safemcp} demonstrate breadth. The 0\% false positive rate on 466 benign SKILL.md samples demonstrates that aggressive detection need not come at the cost of usability.

The infrastructure to attack AI agents exists today. Every CVE, every incident, every benchmark result in this paper is from the last 12 months \cite{cve6514,cve53773,cvegit,cve49596,sandworm,postmark,minja,servicenow,aixbt}. The collapse of trust is happening now. ATR is one answer --- and it is being adopted. The industry needs many more.
...
\bibitem{atrmegscan}
ATR v1.1.1 Mega Scan: 53,577 skills across OpenClaw and Skills.sh, April 8, 2026.

\bibitem{atrskillbench}
ATR SKILL.md Benchmark: 498 real-world skill samples (32 malicious + 466 benign), April 2026.

\bibitem{ciscopr}
Cisco AI Defense skill-scanner: ATR upstream integration, PR \#79, \url{https://github.com/cisco/ai-defense/pull/79}, March 2026.

\bibitem{safemcp}
SAFE-MCP Framework, OpenSSF, \url{https://github.com/anthropics/safe-mcp}, 2026.

\bibitem{snykinvariant}
Snyk acquisition of Invariant Labs, 2026. mcp-scan integrated into Snyk product line.

\bibitem{glasswing}
Project Glasswing, Anthropic + Cisco + Microsoft + CrowdStrike + NVIDIA + Google + Apple, April 2026. Claude Mythos zero-day discovery capability.
