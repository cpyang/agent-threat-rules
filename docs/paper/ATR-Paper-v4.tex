...
\textbf{(5)} We present Agent Threat Rules (ATR) \cite{atr}, the first open-source implementation of this standard, and validate it empirically: 108 YAML-based detection rules across 9 threat categories, independently benchmarked at 99.6\% precision and 61.4\% recall against the external PINT dataset \cite{pint} (850 samples), with 100\% precision and 96.9\% recall on the SKILL.md structured content benchmark (498 samples). A mega scan of the agent skill ecosystem \cite{atrscanmega} (53,577 skills across OpenClaw and Skills.sh) found \textbf{946 flagged skills (1.77\%), with 875 rated CRITICAL} --- and the threat crystallization flywheel has been validated in production with 42 community proposals and 926 threat reports. ATR rules have been adopted as upstream detection by Cisco AI Defense \cite{ciscopr79}.
\end{abstract}
...
\textbf{Self-test corpus:} 341 samples embedded alongside ATR rules, executed as part of the CI pipeline. 824/824 individual test assertions pass across all 108 rules.

\textbf{PINT benchmark:} 850 samples from Lakera's external prompt injection dataset \cite{pint} --- questions ATR has never seen. The 0.25\% false positive rate (1 in 400) means ATR can run on production traffic without alert fatigue. Recall improved from 39.9\% (v0.3.1, 61 rules) to 61.4\% (v1.1.1, 108 rules) through community-contributed patterns and threat crystallization.

\textbf{SKILL.md benchmark:} 498 samples from real-world MCP skill manifests (32 confirmed malicious, 466 verified benign). 100\% precision, 96.9\% recall, 0\% false positive rate. This benchmark tests ATR against its primary operational target: structured skill definitions with defined fields. The 96.9\% recall on structured content vs.\ 61.4\% on adversarial natural language demonstrates the structural advantage of scanning below the reasoning layer --- where attackers cannot freely paraphrase because tool descriptions must follow a schema.


\subsection{Why the Recall Gap Is a Feature, Not a Bug}

ATR achieves 96.9\% recall on structured SKILL.md content and 61.4\% on adversarial natural language (PINT). This gap is informative, not embarrassing:

\begin{lstlisting}
Recall by input type:

  Structured skill manifests (SKILL.md):     96.9%  <-- Schema-constrained,
                                                         limited paraphrase space
  Adversarial natural language (PINT):        61.4%  <-- Infinite paraphrase space,
...
\subsection{Mega Scan: Agent Skill Ecosystem}

In April 2026, we used ATR v1.1.1 to conduct a mega scan of the agent skill ecosystem \cite{atrscanmega}. This is, to our knowledge, the largest automated security audit of AI agent skills to date.

\textbf{Methodology:} Scanned 53,577 skills across two major registries: OpenClaw (50,485 skills) and Skills.sh (3,115 skills, including SKILL.md manifests). Each skill was evaluated against all 108 ATR rules. Findings were reported to Threat Cloud for community review.

\textbf{Results:}

\begin{table}[htbp]
\centering
\caption{Mega Scan: Agent Skill Ecosystem (53,577 skills)}
\label{tab:megascan}
\begin{tabular}{lrr}
\toprule
\textbf{Metric} & \textbf{Count} & \textbf{Percentage} \\
\midrule
Skills flagged (any severity) & 946 & \textbf{1.77\%} \\
CRITICAL & 875 & 1.63\% \\
HIGH & 52 & 0.10\% \\
MEDIUM & 19 & 0.04\% \\
Clean & 52,631 & 98.23\% \\
Threat reports to Threat Cloud & 926 & --- \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Top detection:} Rule ATR-2026-00121 (tool description poisoning) triggered 674 times, accounting for 71\% of all CRITICAL findings. This confirms that tool description poisoning --- embedding hidden instructions in a skill's description that the AI agent processes as trusted input --- is the dominant attack vector in agent skill marketplaces.

\textbf{Key finding:} The 1.77\% flag rate on the mega scan stands in stark contrast to the 49\% finding rate on the earlier npm MCP ecosystem scan (v0.3.1, 2,386 packages). The difference is methodological: the npm scan counted all behavioral indicators (network requests, filesystem writes); the mega scan counts only pattern-matched threat signatures. Both numbers are valid but measure different things. The npm scan shows that half of MCP packages exhibit security-relevant behavior. The mega scan shows that 1.77\% contain patterns that match known attack signatures --- and 92.5\% of those are CRITICAL.
...
Crystallization flywheel (April 2026):
  ...
  Mega scan: 53,577 skills scanned
...
\textbf{Regex ceiling --- structure-dependent.} Tier 2 pattern matching achieves 96.9\% recall on structured SKILL.md content but 61.4\% on adversarial natural language (PINT). The gap is structural: tool descriptions and skill manifests have schemas that constrain the attacker's paraphrase space; free-form natural language does not. ATR's response: use regex where it works (structured content, known patterns, near-zero false positives, $<$5ms), and escalate free-form content to Tiers 3--4.

\textbf{Known evasion techniques.} We have documented 64 evasion techniques across all 108 rules. These are published in each rule's \texttt{evasion\_tests} field. Categories include: paraphrase (21), encoding bypass (12), language switching (9), semantic restructuring (8), multi-turn decomposition (6), context window manipulation (4), and other (4). Documenting what we cannot catch is as important as documenting what we can.
...
The 1,184 malicious skills confirmed on ClawHub \cite{antiy}. Over 8,000 MCP servers exposed on the public internet \cite{checkmarx,paloalto}. 30 CVEs in 60 days \cite{checkmarx,cosai}. The 87\% downstream poisoning rate from a single compromised agent \cite{galileo}. Project Glasswing \cite{glasswing} --- a joint initiative by Anthropic, Cisco, Microsoft, CrowdStrike, NVIDIA, Google, and Apple --- signals that the industry recognizes the scale of the problem, but standards for detection rules remain absent from the initiative.

The infrastructure exists to attack AI agents at scale. The infrastructure to defend them is beginning to emerge.
...
We have implemented these requirements as ATR \cite{atr} --- 108 open-source detection rules, a 5-tier cascade engine, and a validated threat crystallization flywheel --- and tested the approach against three benchmarks: PINT \cite{pint} (99.6\% precision, 61.4\% recall), SKILL.md (100\% precision, 96.9\% recall, 0\% false positives), and a mega scan of 53,577 agent skills \cite{atrscanmega} (946 flagged, 875 CRITICAL). The threat crystallization mechanism has produced 42 community proposals from 926 threat reports, and the recall trajectory --- 39.9\% (v0.3.1) to 61.4\% (v1.1.1) --- demonstrates that the evolutionary capability is working.

ATR is no longer just a proposal. Cisco AI Defense has adopted 34 ATR rules as upstream detection \cite{ciscopr79}. The Snyk acquisition of Invariant Labs \cite{snykinvariant} --- creators of the leading MCP scanning tool --- validates the market but makes the case for an open standard more urgent: when the leading open-source scanner becomes proprietary, the community needs an alternative that cannot be acquired away.

The collapse of trust is happening now. Every CVE, every incident, every benchmark result in this paper is from the last 12 months \cite{cve6514,cve53773,cvegit,cve49596,sandworm,postmark,minja,servicenow,aixbt}. ATR is one answer --- and it is being adopted. The industry needs many more.
...
\bibitem{atrscanmega}
ATR v1.1.1 Mega Scan: 53,577 skills across OpenClaw and Skills.sh, April 2026.
